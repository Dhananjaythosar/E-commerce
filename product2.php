...
<body>
    ...
    <div class="row row2">
        ...
        <img src="<?=$row['product_img']?>" class="card-img-top" height="250"> 
         <div class="panel">
         <form id="addwish" action="wishlist.php" class="form-submit" method="POST">
                     ...
                     <input type="hidden" name="pcode" value="<?=$row['product_code']?>">
                        <a href="#" class="fa fa-heart" type="submit " id="add_to_wish" name="add_to_wish"></a>
                        <a href="#" class="fa fa-share" type="submit " id="add_to_" name="add_to_"></a>
                        <a href="#" class="fa fa-search" type="submit" id="add_to_" name="add_to_"></a>
                    </form>
                    </div>
...
var form = $('#addwish');
$('#add_to_wish').clike(function({
    $.ajax({
        url:form.attr("action"),
        type:'POST',
        data$("#addwish  input").serialize(),
        success: function(data){
            consol.log(date);
            $"(#message").html(response){
